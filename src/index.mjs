...
const startManager = async () => {
  ...
  rline.on('line', (text) => {
    switch (text.split(' ')[0]) {
      ...
      case 'mv':
        if (text.split(' ')[1] === undefined || text.split(' ')[2] === undefined) {
          process.stdout.write(`Invalid input. If you want to move a file, please type mv filename dirname\n`);
          break;
        }
      ...
      case 'os':
        if (text.split(' ')[1] === undefined) {
          process.stdout.write(
            `Invalid input. If you want to learn some info about your system please use this command with args\n`
          );
          break;
        }
        ops(text.split(' ')[1]);
